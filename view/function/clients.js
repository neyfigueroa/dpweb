  ...
  try {
    ...
    if (json.status && json.data) {
      json.data.forEach((user, index) => {
        // Normalizar rol: si es numérico lo tratamos como 'cliente'
        let rolRaw =
          user.rol !== undefined && user.rol !== null
            ? String(user.rol).trim()
            : "";
        let rolLabel =
          rolesMap[rolRaw] ||
          (rolRaw.match(/^\d+$/)
            ? "Cliente"
            : rolesMap[rolRaw.toLowerCase()] || "Desconocido");
        ...
        fila.innerHTML = `
                    ...
                    <td>
                        <a href="${base_url}edit-client/${
          user.id
        }" class="btn btn-outline-primary btn-sm">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        <button onclick="eliminarClients(${
                          user.id
      ...
      content_clients.innerHTML = `
                <tr>
                    <td colspan="7" class="text-center">${
                      json.msg || "No hay clientes registrados"
                    }</td>
                </tr>
            `;
    }
...
async function edit_client(id) {
  try {
    let respuesta = await fetch(
      base_url + "control/UsuarioController.php?tipo=obtener_usuario&id=" + id
    );
    let json = await respuesta.json();
    if (!json.id) {
...
async function actualizarCliente() {
  ...
  let respuesta = await fetch(
    base_url + "control/UsuarioController.php?tipo=actualizar_usuario",
    {
      method: "POST",
      body: datos,
    }
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        let respuesta = await fetch(
          base_url +
            "control/UsuarioController.php?tipo=eliminar_usuario&id=" +
            id
        );
        let json = await respuesta.json();
        if (json.status) {
...
if (document.getElementById("btn_guardar_cambios")) {
  document
    .getElementById("btn_guardar_cambios")
